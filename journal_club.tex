...
\textbf{GAN: two networks compete}
\DenseBullets{
  \item \textbf{Generator:} creates a synthetic image.
  \item \textbf{Discriminator:} decides whether an image is real or generated.
  \item The generator improves by learning to fool the discriminator.
}
\textbf{CDCGAN: conditioned and convolutional}
\DenseBullets{
  \item \textbf{Conditional:} a demand image controls which pressure image is generated.
  \item \textbf{Deep convolutional:} CNN layers learn spatial pressure patterns.
}
\textbf{Here:} demand image $\rightarrow$ expected leak-free pressure image.
\end{column}
\end{columns}
\end{SWNFrame}
...
\textbf{Train normal behavior}
\JCBullets{
  \item U-Net generator predicts leak-free pressure from demand.
  \item PatchGAN and L1 losses make the prediction realistic and numerically close to the target.
  \item Leak examples are not required for training.
}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Compare prediction with observation}
\JCBullets{
