...
% 1:00 -- Cemal
\begin{SWNFrame}{THE OPERATIONAL PROBLEM}
\begin{columns}[T,onlytextwidth]
\begin{column}{0.51\textwidth}
\textbf{Why leak detection is difficult}
\JCBullets{
...
\textbf{Knowledge gap}
\JCBullets{
  \item Existing data-driven methods struggle with labels and localization.
  \item Model-based methods require calibration and repeated hydraulic simulation.
  \item A useful method must be fast, uncertainty-aware, spatial, and trainable without labeled leaks.
}
\end{column}
\end{columns}
\vspace{0.04in}
\textbf{Why it matters:} localization should narrow the field search area before costly inspection.
\LitSource{Context synthesized from Rajabi et al. (2023), pp. 1--3; Wan et al. (2022).}
\end{SWNFrame}

% 1:15 -- Cemal
\begin{SWNFrame}{RESEARCH QUESTION AND CLAIM}
\begin{columns}[T,onlytextwidth]
...
\vspace{0.16in}
\textbf{Testable expectations}
\DenseBullets{
  \item Predicted normal pressure images should match leak-free observations.
  \item A leak should lower global image similarity.
  \item The lowest local similarity should occur near the leak.
}
\end{column}
\begin{column}{0.39\textwidth}
\textbf{Authors' main claim}
\JCBullets{
  \item The CDCGAN + SSIM pipeline is accurate enough for real-time leak detection and localization.
  \item Image representation allows sparse, variable-size monitoring data to scale.
  \item GAN inference is cheaper than repeated hydraulic simulation.
}
\end{column}
\end{columns}
\PaperSource{pp. 1 and 3}
\end{SWNFrame}
