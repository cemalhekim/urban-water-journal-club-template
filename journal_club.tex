...
The demand image controls which pressure image is generated.

\vspace{0.04in}
\textbf{Generator vs. discriminator}

\textbf{U-Net generator:} predicts the leak-free pressure image.

\textbf{PatchGAN discriminator:} classifies local pressure patches as real or generated.

\vspace{0.04in}
\textbf{Training:} the discriminator learns to find fakes; the generator learns to fool it. L1 loss keeps the prediction close to the true image.

\vspace{0.04in}
\textbf{After training:} keep the generator and compare predicted normal with observed pressure using SSIM.
\end{column}
\end{columns}
\end{SWNFrame}
