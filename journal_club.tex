...
% 1:05 -- Moyang
\begin{SWNFrame}{CRITICAL ASSESSMENT AND PRACTICAL ASSUMPTIONS}
\begin{columns}[T,onlytextwidth]
...
\DenseBullets{
  \item Calibrated network with stable topology and pipe characteristics.
  \item Accurate, synchronized pressure data and no concurrent leaks.
}
\end{column}
\begin{column}{0.48\textwidth}
...
\DenseBullets{
  ...
  \item Robustness to sensor faults, operational changes, network drift, or simultaneous leaks.
  \item Generalization beyond one simulated network and its kriging/scaling choices.
}
\vspace{0.04in}
\textbf{Practical risk}
\DenseBullets{
  \item Domain shift may trigger false alarms or corrupt localization.
  \item A fixed $3\sigma$ threshold may not reflect utility-specific alarm costs.
}
\end{column}
\end{columns}
\vspace{0.025in}
\textbf{Validity judgment:} credible as a proof of concept, but practical-readiness claims exceed the presented evidence.
