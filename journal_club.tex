...
\title{Leak detection and localization in water distribution networks using conditional deep convolutional generative adversarial networks}
\author{Cemal Hekim, Jan Luckey, and Moyang Chen}
...
\SWNTitleSlide
  {Leak detection and localization\\in WDNs using conditional deep\\convolutional GANs}
  {Cemal Hekim, Jan Luckey, and Moyang Chen}
  {Rajabi et al., Water Research 238 (2023), 120012}
  { -- critical paper presentation}

\begin{SWNFrame}{PRESENTATION ROADMAP}
\JCBullets{
  \item Research challenges
  \item Paper goal and research question
  \item Proposed methodology and core pipeline
  \item Evaluation design and key results
  \item Critical assessment and assumptions
  \item Conclusion and take-home message
}
\end{SWNFrame}
...
% 0:45 -- Jan
\begin{SWNFrame}{WHAT ARE GAN AND CDCGAN?}
\begin{columns}[T,onlytextwidth]
\begin{column}{0.58\textwidth}
\centering
\PaperAsset[width=\linewidth]{google-gan-diagram.png}
\LitSource{Figure: Google for Developers, ``Overview of GAN Structure,'' CC BY 4.0; definitions: Mirza and Osindero (2014).}
\end{column}
\begin{column}{0.38\textwidth}
\textbf{GAN: adversarial learning}

\textbf{Generator:} creates synthetic samples.

\textbf{Discriminator:} separates generated samples from real data.

\textbf{Competition:} its feedback trains the generator to produce more realistic outputs.

\vspace{0.06in}
\textbf{CDCGAN: the paper's extension}

\textbf{Conditional:} demand controls the generated pressure image.

\textbf{Deep convolutional:} CNN layers learn spatial pressure patterns.
\end{column}
\end{columns}
\end{SWNFrame}
...
\centering
\PaperAsset[height=2.05in,keepaspectratio]{fig10-ssimo.png}

\vspace{0.07in}
\PaperAsset[height=0.95in,keepaspectratio]{fig11-local-ssim.png}
...
% 1:05 -- Moyang
\begin{SWNFrame}{CRITICAL ASSESSMENT}
\begin{columns}[T,onlytextwidth]
\begin{column}{0.48\textwidth}
\textbf{What the evidence supports}
\DenseBullets{
  \item Detection and approximate localization of large, isolated leaks.
  \item Fast inference without online hydraulic simulation.
  \item Similar aggregate results using only 33 sensors.
}
\vspace{0.04in}
\textbf{Key assumptions}
\DenseBullets{
  \item Calibrated, operationally stable network.
  \item Reliable pressure data and no concurrent leaks.
}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{What remains unproven}
\DenseBullets{
  \item GAN advantage over U-Net or residual baselines.
  \item Robustness to faults, operational changes, and network drift.
  \item Generalization beyond one simulated network.
}
\vspace{0.04in}
\textbf{Practical risk}
\DenseBullets{
  \item Domain shift may cause false alarms or localization errors.
  \item A fixed $3\sigma$ threshold may not match utility costs.
}
\end{column}
\end{columns}
\vspace{0.015in}
\textbf{Verdict:} credible proof of concept; deployment readiness remains unproven.
\LitSource{Assumptions reported by Rajabi et al. (2023), pp. 6--13; deployment consequences are our interpretation.}
\end{SWNFrame}

% 1:00 -- Moyang
\begin{SWNFrame}{CONCLUSION, IMPLICATIONS, AND NEXT STEPS}
\begin{columns}[T,onlytextwidth]
\begin{column}{0.48\textwidth}
...
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Priority next steps}
\DenseBullets{
  \item Compare CDCGAN with U-Net and hydraulic-residual baselines.
  \item Test sensor faults, topology changes, drift, and simultaneous leaks.
  \item Validate across multiple networks and real utility measurements.
  \item Calibrate thresholds to delay and false-alarm costs.
}
\end{column}
\end{columns}
\vspace{0.08in}
\textbf{Take-home verdict:} a promising proof of concept whose practical value now depends on comparative baselines and field robustness tests.
\vspace{0.12in}
\centering
\textbf{Questions?}
